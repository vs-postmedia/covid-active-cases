...
const addLabels = (svg, config, data) => {
  config.chart_variables.forEach(d => {
    ...
    const bbox = d3["i" /* select */](`.${d} path`).node().getBBox();
...
const drawData = (svg, metric, i, data, config) => {
  ...
  svg.append('g').attr('class', `area ${metric}`).append('path').datum(variable).attr('stroke', config.fill_colours[i]).attr('stroke-width', 2).attr('opacity', 1).attr('fill', config.fill_colours[i]).attr('opacity', opacity).attr('d', d3["a" /* area */]().x(d => x(d.date)).y0(y(0)).y1(d => y(d.value))); // outline the areas

  svg.append('path').datum(variable).attr('fill', 'none').attr('stroke', config.fill_colours[i]).attr('stroke-width', 2).attr('d', d3["e" /* line */]().x(d => x(d.date)).y(d => y(d.value)));
};
...
const setupFooter = config => {
  const footer = d3["i" /* select */](config.id).append('footer');
...
const setupHeader = (config, data) => {
  ...
  const dateString = ` ${date.toLocaleString('default', {
    month: 'long'
  })} ${date.getDay()}, ${date.getFullYear()}.`; // const dateString = ` ${date.toLocaleString('default', { month: 'long' })} ${date.getDay()}, ${date.getFullYear()}.`
  // add timestamp to deck

  subhead += dateString;
  d3["i" /* select */](config.id).append('header').append('h1').attr('class', 'headline').text(config.headline);
  d3["i" /* select */](`${config.id} header`).append('p').attr('class', 'subhead').html(subhead);
};
...
const ySetup = data => {
  return d3["g" /* scaleLinear */]().domain([0, d3["f" /* max */](data, d => parseInt(d[yScaleMetric]))]).nice().range([height - margin.bottom, margin.top]);
};

const xSetup = data => {
  return d3["h" /* scaleUtc */]().domain(d3["d" /* extent */](data, d => d.date)).range([margin.left, width - margin.right]);
};

const xAxis = g => {
  g.attr("transform", `translate(0, ${height - margin.bottom})`).attr('class', 'x-axis').call(d3["b" /* axisBottom */](x).ticks(5).tickSizeOuter(0).tickFormat(d3["k" /* utcFormat */]('%b')));
};
...
const updateChart = (data, config) => {
  ...
  height = d3["i" /* select */](id).style('height').slice(0, -2) / 1.6 - margin.top - margin.bottom;
  width = d3["i" /* select */](id).style('width').slice(0, -2); // svg

  const svg = d3["i" /* select */](id).append('svg').attr('viewBox', [0, 0, width, height]); // Add axes
